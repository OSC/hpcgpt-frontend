  ...
  const deleteDocumentMutation = useMutation({
    mutationFn: async (recordsToDelete: CourseDocument[]) => {
      ...
      const deletePromises = recordsToDelete.map((record) =>
        axios.delete(`/api/OSC-api/deleteDocument`, {
          params: {
  ...
  const showToastOnFileDeleted = (theme: MantineTheme, was_error = false) => {
    return (
      ...
      notifications.show({
        ...
        message: was_error
          ? "An error occurred while deleting the file. Please try again and I'd be so grateful if you email rohan13@osc.edu to report this bug."
